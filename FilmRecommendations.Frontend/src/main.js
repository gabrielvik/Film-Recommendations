...
promptForm.addEventListener('submit', (e) => {
  ...
  setTimeout(() => {
    ...
    // Save the movie list so that when the user returns, results can be re‑rendered.
...
promptInput.addEventListener('input', () => {
  ...
  // If the query is changed or emptied, clear the results and update the stored query
  if (currentQuery !== lastSearchQuery) {
    movieRecommendations.innerHTML = '';
    sessionStorage.removeItem('movieRecommendations');
    // Optionally, update the lastSearchQuery for future comparisons
